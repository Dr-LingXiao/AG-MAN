...
parser.add_argument('--dataset', default="DeepFashion", type=str,
...
def train(train_loader, tnet, criterion, optimizer, epoch,awl):
    ...
    for batch_idx, (data1, data2, data3, c) in enumerate(train_loader):
        ...
        # compute similarity
        sim_a, sim_b, loss_c = tnet(data1, data2, data3, c)
...
def test(test_candidate_loader, test_query_loader, test_model, epoch=-1):
    ...
    for _, (img, c, gdtruth, _) in enumerate(test_candidate_loader):
        ...
        masked_embedding, _ = test_model(img, c)
...
def save_checkpoint(state, is_best, filename='checkpoint.pth.tar'):
    ...
    directory = "runs_DEEPFASHION/%s/"%(args.name)
    if not os.path.exists(directory):
        os.makedirs(directory)
    filename = directory + filename
    torch.save(state, filename)
    if is_best:
        shutil.copyfile(filename, 'runs_DEEPFASHION/%s/'%(args.name) + 'model_best.pth.tar')
...
def main():
    ...
    # backbone = resnet()
    backbone = resnet.resnet50(len(attributes))
    enet = get_model(args.model)(backbone, n_attributes=len(attributes), embedding_size=args.dim_embed)
    tnet = get_model('Tripletnet')(enet, criterion1,len(attributes))
    if args.cuda:
        tnet.cuda()
        awl.cuda()
